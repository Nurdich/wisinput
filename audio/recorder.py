...
class AudioRecorder:
    def __init__(self):
        ...
        logger.info(f"初始化完成")
        # 实时电平回调（在录音回调中触发）。签名：callback(level_float_0_1)
        self.level_callback = None
        # 实时样本回调（在录音回调中触发）。签名：callback(samples_float_array)
        # samples 为一维 float32，幅度约在 [-1, 1]
        self.samples_callback = None
                ...
                def audio_callback(indata, frames, time, status):
                    ...
                    if self.recording:
                        ...
                        try:
                            ...
                            # indata 通常为 float32，幅度约在 [-1,1]；做一个 clamp
                            level = max(0.0, min(1.0, rms))
                            if callable(self.level_callback):
                                self.level_callback(level)
                            # 推送波形样本（仅取单通道）
                            if callable(self.samples_callback):
                                try:
                                    mono = indata[:, 0] if indata.ndim > 1 else indata
                                    # 复制出回调数据，避免后续复用缓冲影响
                                    self.samples_callback(np.asarray(mono, dtype=np.float32).copy())
                                except Exception:
                                    pass
                        except Exception:
                            pass
    ...
    def set_level_callback(self, callback):
        """设置录音时的电平回调。签名：callback(level_0_1: float)"""
        self.level_callback = callback

    def set_samples_callback(self, callback):
        """设置录音时的样本回调。签名：callback(samples: np.ndarray[float32])"""
        self.samples_callback = callback
